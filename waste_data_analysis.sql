...
-- city_data columns: iso3c, country_name, city_name, city_code, income_id, region_id, food_pct, plastic_pct,
    -- paper_pct, glass_pct, metal_pct, textile_pct, other_pct, recycling_pct, open_dump_pct,
    -- sanitary_landfill_pct, income_label

-- country_data columns: iso3c, country_name, income_id, region_id, food_pct, plastic_pct, paper_pct,
    -- glass_pct, metal_pct, textile_pct, other_pct, open_dump_pct, recycling_pct,
    -- sanitary_landfill_pct, income_label
...
-- I group cities by income level and calculate the average % of food, plastic, paper, and textile in their waste.
...
SELECT
    ...
    ROUND(AVG(c.textile_pct), 3) AS avg_textile_pct
FROM city_data c
...
-- QUERY 3: Does waste composition predict open dumping and recycling rates?
-- This helps answer the second part of my research question.
-- I join city-level food waste % with country-level treatment rates, grouped by income group.
...
-- sanitary_landfill_pct is included as the "good" disposal option — higher income countries
-- are expected to use more sanitary landfills instead of open dumps.
...
SELECT
    ...
    ROUND(AVG(n.sanitary_landfill_pct), 3) AS avg_country_sanitary_landfill_pct,
    COUNT(c.city_code)                     AS number_of_cities
FROM city_data c
JOIN country_data n ON c.iso3c = n.iso3c
WHERE c.income_id IS NOT NULL
...
-- QUERY 4: Which regions recycle the most, dump the most, and use sanitary landfills the most?
-- Shows how all three treatment outcomes differ by world region,
-- using country-level treatment data joined to city-level data.
-- This gives a complete picture of waste treatment across regions.
